# TRADEMARKS

**R-Think™ Trademark Policy**
**Status:** PENDING GUARDIAN AND HUMAN ARCHITECT ACCEPTANCE

## Founding Leadership

### Founder

**Hendri RH — Bro Kraken**
Framework Creator, Human Architect, Final Doctrine Authority

### Co-Founder

**Bro CG**
Architecture Guardian, Engineering Coordinator

### Co-Founder

**Hatmadita Ramuny**
Public Relations, Community Relations, Partnership Development
LinkedIn: https://www.linkedin.com/in/hatmadita/

---

## Project Identifiers

The following are claimed project identifiers:

- **R-Think** — project and framework name
- **R-Think™** — public wordmark

The R-Think name and logo are excluded from the software and documentation
license grants and are governed separately by this Brand and Trademark Usage
Policy. No completed government trademark registration is claimed.

The ™ symbol does **not** represent completed government trademark registration.
No DJKI (Direktorat Jenderal Kekayaan Intelektual) registration is claimed as
completed.

## License Separation

The software license (AGPL-3.0-only) and documentation license (CC-BY-SA-4.0)
do **not** grant unrestricted trademark rights. The following are reserved
separately:

- R-Think name
- R-Think™ wordmark
- R-Think logo
- Official certification claims
- Official compatibility claims
- Official endorsement claims
- Confusingly similar branding

## Permitted Use

- **Nominative factual reference:** You may refer to R-Think factually in
  descriptive contexts (e.g., "This project uses the R-Think framework")
- **Factual compatibility:** You may state factual compatibility without
  implying official endorsement

## Prohibited Use Without Permission

- **Implied official status:** Forks and derivatives must not imply official
  status, certification, sponsorship, or endorsement
- **Confusing branding:** Modified versions must use sufficiently distinct
  branding unless written permission is granted
- **Logo use:** Logo use requires explicit permission except for factual
  reference where legally allowed
- **® symbol:** Do not use ® unless trademark registration is completed and
  applicable in the relevant jurisdiction

## Modified Versions

If you fork or modify R-Think:

- Use sufficiently distinct branding to avoid confusion
- Do not imply your version is the official R-Think
- Do not use the R-Think™ wordmark in your project name without permission
- Retain attribution to the original project

## Permission Process

Trademark permission requests are currently managed by:

**Hendri RH — Bro Kraken**
Framework Creator and Human Architect

Public Relations and community inquiries:

**Hatmadita Ramuny**
Public Relations, Community Relations
LinkedIn: https://www.linkedin.com/in/hatmadita/

## Legal Disclaimer

This trademark policy does not replace professional legal advice. Trademark
enforceability may depend on jurisdiction, registration status, and factual
ownership. Professional IP counsel is recommended before formal trademark
enforcement.

---

*This trademark policy is part of the R-Think IP foundation established by
RTHINK-IP-001. Founding Leadership structure established by RTHINK-RT-004-C1.*
*It requires Guardian and Human Architect acceptance.*
